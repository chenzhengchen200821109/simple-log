...
ThreadPool::ThreadPool(const string& nameArg, int threads, int size) : name_(nameArg), queue_(0), threads_(threads), size_(size), mutex_(), notEmpty_(mutex_), notFull_(mutex_) ,notFinish_(mutex_)
{
    start();
}
...
// thread pool won't accept new task and try to terminate working threads
