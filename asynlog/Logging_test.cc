...
int g_total;
FILE* g_file;
std::unique_ptr<muduo::LogFile> g_logFile; // log file

void dummyOutput(const char* msg, int len)
{
    g_total += len;
    if (g_file) { // write to a standard file
        fwrite(msg, 1, len, g_file);
    }
    else if (g_logFile.get()) { // or write to a log file on backend
...
void bench(const char* type)
{
    ...
    // calculate time consumed and profile
